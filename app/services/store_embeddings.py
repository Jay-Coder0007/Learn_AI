...
project_root = os.path.abspath(os.path.join(current_dir, "..", ".."))  # ssc_AI

# Add utils path for schema
utils_dir = os.path.join(project_root, "app", "utils")
...
# === Constants ===
COLLECTION_PREFIX = "class_"
EMBEDDINGS_BASE_PATH = os.path.join(project_root, "embeddings")
PROCESSED_BASE_PATH = os.path.join(project_root, "processed")
...
def load_embeddings_and_chunks(class_dir, subject):
    subject_path = os.path.join(EMBEDDINGS_BASE_PATH, f"{class_dir}_{subject}.npy")
    chunks_path = os.path.join(PROCESSED_BASE_PATH, class_dir, subject, "chunks.json")
...
def add_subject_data(collection_name, class_dir, subject, client_id="default"):
    ...
    for i, chunk in enumerate(chunks):
        try:
            chunk_content = chunk if isinstance(chunk, str) else chunk.get("text", chunk.get("content", ""))
            chapter_title = f"Chapter {i+1}" if isinstance(chunk, str) else chunk.get("chapter_title", f"Chapter {i+1}")
...
def main():
    ...
    if args.mode == 'full':
        for class_dir in sorted(os.listdir(PROCESSED_BASE_PATH)):
            if not class_dir.startswith("class_"):
                continue
            ...
            for fname in os.listdir(EMBEDDINGS_BASE_PATH):
                if fname.startswith(class_dir + "_"):
                    subject = fname[len(class_dir)+1:-4]
                    subjects.add(subject)
