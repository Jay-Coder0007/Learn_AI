...
def run_cleaning():
    # Get root project directory from current script location
    ...
    for class_dir in os.listdir(output_root):
        class_path = os.path.join(output_root, class_dir)
        if not os.path.isdir(class_path): 
            continue
        ...
        for filename in os.listdir(class_path):
            ...
            # Save cleaned text in processed directory
            subject_dir = os.path.join(processed_root, class_dir, subject)
            os.makedirs(subject_dir, exist_ok=True)
